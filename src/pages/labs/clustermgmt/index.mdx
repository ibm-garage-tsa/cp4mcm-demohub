...
17.Enter **microk8s** for cluster name (1) and **microk8s** for namespace (2). Click **Generate command** to continue (3).
...
19.Go back to the desktop and open the terminal window to MicroK8s cluster clicking the **MicroK8s Terminal** link. 

![](images/2020-09-17-20-30-28.png)

20.MicroK8s terminal has a yellow background. To verify the cluster status run the following command in the **MicroK8s** window
...
23.Back to the browser window, click **View cluster** and make sure that the cluster status is **Ready** now (if necessary, refresh the details page).
...
25.Now you can see your both clusters in a clusters list. You can add labels to identify your new cluster. Follow the same procedure as in steps 13-14 above to add a label **environment** = **QA** to the microK8s cluster
...
16.Great! Your Liberty application is available on your managed cluster. 

  ![](images/2020-09-18-20-31-41.png)

  This application is really simple sample, just to show you how easy is to install an application from the catalog in multiple clusters. In our next lab you will deploy a business application (ModResorts) learning details about Application Management deployment using channels and subscriptions concepts.
