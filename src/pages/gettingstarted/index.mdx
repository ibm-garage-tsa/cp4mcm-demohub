...
![alt text again](./images/clustermgmt-banner.jpg)
...
## Accessing the desktop
  ...
  If you cannot see these menus check if there are any [hanging pods](#some-pods-hangs-on-startup).

  Otherwise, environment is ready to start the excercises.
...
### Worker node registered as localhost
  ...
  To correct the situation, open the terminal window, login to OCP with the `./oclogin.sh` command and run the following command `oc delete node localhost`

  Then stop the whole environment and start it again.
